...
select dr_drvname, dr_salary, dr_drvstatus from driver
...
select dr_drvname, dr_hiredate from driver
...
select ca_cabnum, ca_make, ca_model
from cab
...
SELECT DR_DRVNAME, CA_MAKE, CA_MODEL
FROM DRIVER
...
SELECT DISTINCT DR_DRVNUM, DR_DRVNAME, IC_CABNUM, IC_INCITYPE
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
...
SELECT CA_CABNUM, CA_MAKE, CA_MODEL, CA_YEAR
FROM CAB
...
select distinct dr_drvname, (sysdate-dr_brthdate)/365.25 AS AGE
from driver
...
/
11. Display the names of all drivers who have a jobcode of 52 and who have a driver status other than "Do Not Reserve". Please display the first name followed by the middle initial and the last name of each driver. Suggestion: Don’t worry about displaying the first name, middle initial and last name of the driver at first. We’ve discuss the SUBSTR and INSTR function towards the end of the semester. Start out by writing a query that satisfies the two conditions. 
SELECT DR_DRVNUM, DR_DRVNAME
FROM DRIVER
...
SELECT SUBSTR(TO_CHAR(DR_BRTHDATE, 'YYYY'), 1, 4) AS BIRTH_YEAR, COUNT(*) AS NUM_DRIVERS
FROM DRIVER
...
select dr_drvname, sh_wkdate, sh_wkshift
from driver, shift
...
SELECT SH_WKDATE, SH_WKSHIFT, SUM(SH_ODOMRDG2 - SH_ODOMRDG1) AS Total_Miles
FROM SHIFT
WHERE SH_WKFLAG <> 'F'
...
SELECT SH_WKDATE, SH_WKSHIFT, AVG(SH_ODOMRDG2 - SH_ODOMRDG1) AS Average_Miles
FROM SHIFT
...
select ca_cabnum, ca_make, ca_purdate from cab
...
SELECT DR_DRVNAME, (CURRENT_DATE - DR_BRTHDATE) / 365.25 AS Age, (CURRENT_DATE - DR_HIREDATE) / 365.25 AS Tenure
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
...
select ca_cabnum, ca_make, ca_model, ca_year, ma_maintype
from cab, maintain
...
select SUBSTR(ca_licnum, 1, 2) || ' ' || SUBSTR(ca_licnum, 3) AS formatted_plate, ca_cabnum, ca_year, ca_make
from cab
...
select dr_drvname, dr_drvcity, dr_salary
from driver
...
select dr_drvname, dr_salary, dr_jobcode, round ((sysdate-dr_brthdate)/365.25,0) AS AGE, round((sysdate-dr_hiredate)/365.25,2) AS LengthOfService
from driver
...
select dr_drvname as"Unqualified Drivers"
...
select ca_cabnum, ca_make, sum(ma_cost)
...
SELECT DR_DRVNUM, DR_DRVNAME, (DATE '2010-12-01' - DR_HIREDATE) / 365.25 AS Tenure, (DR_HIREDATE - DR_BRTHDATE) / 365.25 AS Age_At_Hire
FROM DRIVER
...
select ma_cabnum, ma_maindate, ma_maintype, ma_mainodom, sum(ma_cost), SUBSTR(ca_licnum, 3, 8) AS plate_numbers
from  maintain, cab
...
select sh_wkdate, sh_wkshift, count(sh_cabnum)
from shift
group by sh_wkdate, sh_wkshift
/
31. Display all drivers with a "Pay Before" status whose monthly salary is between $4000 and $4700. 
select dr_drvname, dr_drvstatus, round((dr_salary/12),2) AS  "monthly salary"
from driver
...
SELECT SUBSTR(TO_CHAR(DR_HIREDATE, 'YYYY'), 1, 4) AS HIRE_YEAR,
...
select dr_drvname, round (dr_salary/12)
from driver
...
select dr_drvname, dr_drvcity, dr_drvstate, dr_drvzip
from driver
...
SELECT DR_DRVNAME, DR_SALARY, DR_DRVSTATUS
FROM DRIVER
WHERE DR_SALARY < (
  SELECT AVG(DR_SALARY)
  FROM DRIVER
...
select MAX(dr_brthdate), MIN(dr_brthdate), (MAX(DR_BRTHDATE)-MIN(dr_brthdate))/365 AS Age_difference_Years
from driver
/
...
SELECT CA_CABNUM, NVL(SUM(SH_NUMFARES * CA_BASEFARE + (SH_ODOMRDG2 - SH_ODOMRDG1) * CA_MILEAGECHARGE), 0) AS TOTAL_FARES
FROM CAB
...
SELECT DR_DRVNAME, SUM(SH_ODOMRDG2 - SH_ODOMRDG1) AS Total_Miles
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
...
SELECT SH_WKDATE, SH_WKSHIFT, SH_CABNUM, SH_DRVNUM, (SH_NUMFARES * CA_BASEFARE) + ((SH_ODOMRDG2 - SH_ODOMRDG1) * CA_MILEAGECHARGE) AS Total_Fare
FROM SHIFT
JOIN CAB ON SHIFT.SH_CABNUM = CAB.CA_CABNUM
WHERE SH_WKFLAG = 'T'
/
...
/
45. For each driver involved in three or more shifts, display the driver name and the number of shifts either driven or scheduled to be driven. Order the results in descending order by number of shifts driven. SELECT DR_DRVNAME, COUNT(*) AS Shift_Count
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
GROUP BY DR_DRVNAME
HAVING COUNT(*) >= 3
...
46. Display the number of maintenance activities performed on each day in ascending order by maintenance date. 
select count(ma_maintype), ma_maindate
from maintain
group by ma_maindate
order by ma_maindate
/
47. Display the cab number, make, and model of all cabs either driven or scheduled to be driven on two or more shifts. Sh_wkflag can be ignored on this query. 
SELECT CA_CABNUM, CA_MAKE, CA_MODEL
FROM CAB
...
HAVING COUNT(*) >= 2
/
48. Calculate the average fuel capacity for each make of cab. Display the make and average fuel capacity. Order the result in descending order by average fuel capacity. 
SELECT CA_MAKE, AVG(CA_FUELCAPACITY) AS Average_Fuel_Capacity
FROM CAB
GROUP BY CA_MAKE
ORDER BY Average_Fuel_Capacity DESC
/
49. What are the names of the drivers involved in at least one incident? For each driver display the number of incidents in which he or she has been involved. Order the results in descending order by number of incidents. 
SELECT DR_DRVNAME, COUNT(*) AS NUM_INCIDENTS
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
JOIN INCIDENT ON SHIFT.SH_CABNUM = INCIDENT.IC_CABNUM AND SHIFT.SH_WKDATE = INCIDENT.IC_WKDATE AND SHIFT.SH_WKSHIFT = INCIDENT.IC_WKSHIFT
GROUP BY DR_DRVNAME
...
/
50. Display the number of incidents associated with each driver. If a driver has never been involved in an incident, display a zero. (Suggestion: Use of an outer join might be helpful here). Order the results in descending order by number of incidents. 
SELECT DR_DRVNAME, COUNT(IC_INCITYPE) AS NUM_INCIDENTS
FROM DRIVER
...
ORDER BY NUM_INCIDENTS DESC
/
51. Display the total of the fares for each driver. Include all drivers – even those who have never driven a shift. This query should involve the same calculation of the fare for a shift as that used in query 37.
SELECT DR_DRVNAME, SUM((SH_NUMFARES * CA_BASEFARE) + ((SH_ODOMRDG2 - SH_ODOMRDG1) * CA_MILEAGECHARGE)) AS Total_Fare
FROM DRIVER
LEFT JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
LEFT JOIN CAB ON SHIFT.SH_CABNUM = CAB.CA_CABNUM
GROUP BY DR_DRVNAME
/
52. Calculate the total of the fare for all of the day, all of the evening, and all of the night shifts driven. In other words, we’re looking for three totals – one for the day shift, one for the evening shift, and one for the night shift. 
SELECT SH_WKSHIFT, SUM((SH_NUMFARES * CA_BASEFARE) + ((SH_ODOMRDG2 - SH_ODOMRDG1) * CA_MILEAGECHARGE)) AS Total_Fare
FROM SHIFT
JOIN CAB ON SHIFT.SH_CABNUM = CAB.CA_CABNUM
GROUP BY SH_WKSHIFT
/
53. Calculate the total fuel expenditures for each fuel code. The fuel codes are P – Premium, R – Regular and S – Super. 
SELECT FUL_FUELCODE, SUM(FUL_NUMGAL * FUL_PRICEGAL) AS Total_Fuel_Expenditure
FROM FUELUSE
GROUP BY FUL_FUELCODE
/
54. Count the number of days each cab has been in for maintenance. In addition to the number of days it has been in for maintenance, display the cab number along with its make and model. Include in your result those cabs that have not been in for maintenance at all. Display your results in descending order by days in for maintenance. 
SELECT CA_CABNUM, CA_MAKE, CA_MODEL, COUNT(MA_MAINDATE) AS Days_In_Maintenance
FROM CAB
LEFT JOIN MAINTAIN ON CAB.CA_CABNUM = MAINTAIN.MA_CABNUM
GROUP BY CA_CABNUM, CA_MAKE, CA_MODEL
ORDER BY Days_In_Maintenance DESC
/
55. Display the cab number, license number, make, and model of those cabs that have never received any maintenance? 
SELECT CA_CABNUM, CA_LICNUM, CA_MAKE, CA_MODEL
FROM CAB
LEFT JOIN MAINTAIN ON CAB.CA_CABNUM = MAINTAIN.MA_CABNUM
WHERE MA_MAINDATE IS NULL
/
56. Display the cab number, license number, make, and model of those cabs that have never received any maintenance, been driven on a shift, or been involved in an incident. 
SELECT CA_CABNUM, CA_LICNUM, CA_MAKE, CA_MODEL
FROM CAB
LEFT JOIN MAINTAIN ON CAB.CA_CABNUM = MAINTAIN.MA_CABNUM
LEFT JOIN SHIFT ON CAB.CA_CABNUM = SHIFT.SH_CABNUM
...
/
57. Display the names of drivers who have driven shifts on more than one day along with the number of days on which they have driven. 
SELECT DR_DRVNAME, COUNT(DISTINCT SH_WKDATE) AS Days_Driven
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
WHERE SH_WKFLAG = 'T'
GROUP BY DR_DRVNAME
...
/
58. Display the cabs actually driven by each driver (i.e., Sh_wkflag = 'T'). Include in the results the name of the driver along with the cab number, year, make and model of each cab driven. Order the results by the name of the driver. If a driver has driven a particular cab more than once, display the information requested just once (i.e., use DISTINCT). 
SELECT DISTINCT DR_DRVNAME, CA_CABNUM, CA_YEAR, CA_MAKE, CA_MODEL
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
JOIN CAB ON SHIFT.SH_CABNUM = CAB.CA_CABNUM
WHERE SH_WKFLAG = 'T'
ORDER BY DR_DRVNAME
/
59. Display the total of the maintenance and fuel costs for each cab. In my opinion, this is the most difficult query. I suggest you leave it until the end. 
SELECT MA_MA_CABNUM, SUM(MA_COST) AS TOTAL_MAINTENANCE_COST, SUM(FUL_PRICEGAL * FUL_NUMGAL) AS TOTAL_FUEL_COST, SUM(MA_COST) + SUM(FUL_PRICEGAL * FUL_NUMGAL) AS TOTAL_COST
...
/
60. Display the cab number, make, model and license number of all cabs with either a Texas or Louisiana license number. The result should be displayed in ascending order by license number.
select ca_cabnum, ca_make, ca_model, ca_licnum
from cab
where ca_licnum like 'TX%' or ca_licnum like 'LA%'
order by ca_licnum ASC
/
61. Display all shifts assigned to drivers who have a "DO NOT RESERVE" status. Include in your results the name of the driver and the driver status. 
select dr_drvname, dr_drvstatus, sh_wkdate, sh_wkshift
from driver, shift
where dr_drvnum = sh_drvnum
AND dr_drvstatus like 'Do Not Reserve'
/
62. For each cab, including those never driven on a shift, display the total mileage driven. Order the results in descending order by miles driven. 
SELECT CA_CABNUM, SUM(SH_ODOMRDG2 - SH_ODOMRDG1) AS Total_Mileage
FROM CAB
LEFT JOIN SHIFT ON CAB.CA_CABNUM = SHIFT.SH_CABNUM
GROUP BY CA_CABNUM
ORDER BY Total_Mileage DESC
/
63. Display the number of shifts actually driven (i.e., Sh_wkflag = T) by each driver. The result should include the name of each driver along with the number of shifts driven. 
SELECT DR_DRVNAME, COUNT(*) AS Shifts_Driven
FROM DRIVER
JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
WHERE SH_WKFLAG = 'T'
GROUP BY DR_DRVNAME
/
64. Display the number of shifts actually driven (i.e., Sh_wkflag = T) by each driver, even those who have never driven a shift (i.e., for whom the number of shifts driven is zero). The result should include the name of each driver along with the number of shifts driven. 
SELECT DR_DRVNAME, COUNT(SH_WKFLAG) AS Shifts_Driven
FROM DRIVER
LEFT JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM AND SH_WKFLAG = 'T'
GROUP BY DR_DRVNAME
/
65. The rows in the FUELUSE table have some inaccuracies. Display which rows in the FUELUSE table are associated with instances where the number of gallons of gas exceeds the capacity of the tank. 
SELECT FUELUSE.*
FROM FUELUSE
JOIN CAB ON FUELUSE.FUL_CABNUM = CAB.CA_CABNUM
WHERE FUL_NUMGAL > CA_FUELCAPACITY;
/
 66. Display the names of all drivers who are qualified to drive a Checker cab. Please display the first name followed by the middle initial and the last name of each driver. The substr, instr, and length functions might be of use to you on this query. 
SELECT CONCAT(
...
67. Display the name, salary and age of the driver with the lowest salary among those drivers younger than 40 years of age. 8 
SELECT DR_DRVNAME, DR_SALARY, FLOOR(MONTHS_BETWEEN(SYSDATE, DR_BRTHDATE) / 12) AS Age
FROM DRIVER
WHERE Age < 40
ORDER BY DR_SALARY ASC
FETCH FIRST 1 ROW ONLY
/
68. Display the total fare earned by each driver, even those drivers who have never driven a shift. The result should be displayed in descending order (i.e., the name of the driver who has earned the most should be displayed first, the name of the driver who has earned the second most should be displayed second, etc.). This query should involve the same calculation of the fare for a shift as that used in query 37 
SELECT DR_DRVNAME, SUM((SH_NUMFARES * CA_BASEFARE) + (SH_ODOMRDG2 - SH_ODOMRDG1) * CA_MILEAGECHARGE) AS Total_Fare
FROM DRIVER
LEFT JOIN SHIFT ON DRIVER.DR_DRVNUM = SHIFT.SH_DRVNUM
LEFT JOIN CAB ON SHIFT.SH_CABNUM = CAB.CA_CABNUM
...
/
69. Display the number of incidents associated with each make of cab even those that have never been involved in an incident at all. The results should be in ascending order by make of cab. 
SELECT CA_MAKE, COUNT(IC_INCITYPE) AS Incident_Count
FROM CAB
LEFT JOIN INCIDENT ON CAB.CA_CABNUM = INCIDENT.IC_CABNUM
GROUP BY CA_MAKE
ORDER BY CA_MAKE ASC
/
70. Display the names of those drivers without a middle initial. A person with a name such as W. Lamar Hicks would not a have a middle initial. 
SELECT DR_DRVNAME
FROM DRIVER
WHERE INSTR(DR_DRVNAME, ' ', 1, 2) = 0
/
71. Display the driver number, driver name, cab number, make, and model for all shifts driven using the cab with the smallest fuel capacity. Include as part of your result the date of shift.
SELECT SH_DRVNUM, DR_DRVNAME, SH_CABNUM, CA_MAKE, CA_MODEL, SH_WKDATE
FROM SHIFT
...
/
72. Display the total cost of the maintenance done on cabs currently judged to be in Excellent condition. Include those cabs in Excellent condition that have not been in for maintenance. In other words, an outer join might be of use here. 
SELECT CA_CONDITION, SUM(MA_COST) AS TOTAL_MAINTENANCE_COST
FROM CAB
LEFT JOIN MAINTAIN ON CAB.CA_CABNUM = MAINTAIN.MA_CABNUM
WHERE CA_CONDITION = 'Excellent'
GROUP BY CA_CONDITION
/
73. Display the names of those drivers born in the month of June. 
SELECT DR_DRVNAME
FROM DRIVER
WHERE EXTRACT(MONTH FROM DR_BRTHDATE) = 6
/
74. Display cab number and make of all Checker cabs that have never been involved in an incident. 
SELECT CA_CABNUM, CA_MAKE
FROM CAB
LEFT JOIN INCIDENT ON CAB.CA_CABNUM = INCIDENT.IC_CABNUM
WHERE CA_MAKE = 'Checker' AND IC_INCITYPE IS NULL
/
75. Append the text “Do Not Reserve” to the names of all drivers with a ‘Do Not Reserve’ status. 
SELECT CONCAT(DR_DRVNAME, ' Do Not Reserve') AS Driver_Name
FROM DRIVER
WHERE DR_STATUS = 'Do Not Reserve'
/
76. Display the shifts scheduled to be driven by a driver qualified to drive the cab associated with the shift. Suggestion: you may find the concatenation operator useful in the WHERE clause on this query. 
SELECT SHIFT.*
FROM SHIFT
JOIN QUALIFICATION ON SHIFT.SH_DRVNUM = QUALIFICATION.QU_DRVNUM AND SHIFT.SH_CABNUM = QUALIFICATION.QU_CABNUM
/
77. Display the shifts scheduled to be driven by a driver not qualified to drive the cab associated with the shift. Include the cab number, driver number, driver name, date of the shift and the shift itself in the output. Order the result by driver name. If this query retrieves any rows, it indicates that the database has some data integrity problems. 
SELECT SH_CABNUM, SH_DRVNUM, DR_DRVNAME, SH_WKDATE, SH_WKSHIFT
FROM SHIFT
JOIN DRIVER ON SHIFT.SH_DRVNUM = DRIVER.DR_DRVNUM
...
ORDER BY DR_DRVNAME;
78. Which incidents involve someone driving a cab that he or she is not qualified to drive? For each qualifying incident, display the details of the incident (i.e., cab number, date, shift, type of incident) and the driver number and name of the driver. 
SELECT IC_CABNUM, IC_SHIFTDATE, IC_SHIFT, IC_INCITYPE, DRIVER.DR_DRVNUM, DR_DRVNAME
FROM INCIDENT
...
WHERE QUALIFICATION.QU_DRVNUM IS NULL
/
79. For those cabs receiving fuel during the November 15 – November 18, 2022 time period, display the names of the driver number, driver name, and cab number. 
SELECT DISTINCT DR_DRVNUM, DR_DRVNAME, FU_CABNUM
FROM FUELUSE
JOIN DRIVER ON FUELUSE.FU_DRVNUM = DRIVER.DR_DRVNUM
WHERE FU_FUELDATE BETWEEN '2022-11-15' AND '2022-11-18'
/
80. For each driver qualified to drive cabs, display the number of cabs of each make he or she is qualified to drive. Include in your output the driver number, driver name, cab make, and the number of cabs of that make he or she is qualified to drive.
'SELECT QU_DRVNUM, DR_DRVNAME, CA_MAKE, COUNT(QU_CABNUM) AS Number_of_Cabs
FROM QUALIFICATION
